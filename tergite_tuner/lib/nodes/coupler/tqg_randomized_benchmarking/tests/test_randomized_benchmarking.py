...
from tergite_tuner.lib.nodes.coupler.tqg_randomized_benchmarking.utils.pauli_transfer_matrices import (
    CZ,
    I,
    X_theta,
    Y_theta,
)
from tergite_tuner.lib.nodes.coupler.tqg_randomized_benchmarking.utils.randomized_benchmarking import (
    calculate_net_clifford,
    randomized_benchmarking_sequence,
)

_TEST_PARAMS = [(SingleQubitClifford, 1, 20), (TwoQubitClifford, 2, 10_4368)]


@pytest.mark.parametrize(
    "clifford_cls, clifford_grp, interleaved_clifford_idx", _TEST_PARAMS
)
def test_recovery_clifford_produces_identity(
    clifford_cls, clifford_grp, interleaved_clifford_idx
):
...
def test_interleaved_randomized_benchmarking(
    clifford_cls, clifford_grp, interleaved_clifford_idx
):
    """Test that recovery gate in clifford sequence yields identity."""
    sequence = randomized_benchmarking_sequence(
...
def test_interleaved_cz():
    ...
    for idx in sequence:
        ...
        decomposition = clifford_cls(idx).gate_decomposition
