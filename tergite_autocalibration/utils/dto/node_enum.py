...
that would otherwise form between :mod:`tergite_autocalibration.config.session` (which
references :class:`NodeEnum` for field validation) and
:mod:`tergite_autocalibration.lib.nodes` (which imports every concrete
node class to populate :data:`__NODE_ENUM_CLS_MAP__`).

:mod:`tergite_autocalibration.lib.nodes` re-exports :class:`NodeEnum`
