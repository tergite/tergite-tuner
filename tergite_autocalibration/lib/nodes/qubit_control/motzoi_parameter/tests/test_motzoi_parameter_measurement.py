...
def test_dummy_generation(session_context):
    ...
    node = MotzoiParameterNode(session_context.qubits, session_context.couplers)
    dummy_dataset = node.generate_dummy_dataset()
    first_qubit = session_context.qubits[0]
    ...
    assert len(dummy_dataset.data_vars) == len(session_context.qubits)
    assert dummy_dataset.data_vars[0].size == number_of_reps * number_of_motzois


def test_dummy_12_generation(session_context):
    ExtendedTransmon.close_all()  # ensure no other transmon objects are instantiated
    node = MotzoiParameter12Node(session_context.qubits, session_context.couplers)
    dummy_dataset = node.generate_dummy_dataset()
    first_qubit = session_context.qubits[0]
    number_of_reps = len(node.schedule_samplespace["X_repetitions"][first_qubit])
    number_of_motzois = len(node.schedule_samplespace["mw_motzois"][first_qubit])
...
def test_12_pulse_duration(session_context):
    with loaded_redis(session_context.redis, _redis_values) as rd:
        ExtendedTransmon.close_all()  # ensure no other transmon objects are instantiated
        qubits = ["q13", "q14", "q15"]
