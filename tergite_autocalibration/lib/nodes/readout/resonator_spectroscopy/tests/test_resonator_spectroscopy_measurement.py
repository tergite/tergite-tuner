...
def test_measurement_0_type():
    ...
    node_0 = ResonatorSpectroscopyNode(CONFIG.run.qubits, CONFIG.run.couplers)
    assert issubclass(node_0.measurement_type, ScheduleNode)


def test_measurement_1_type():
    ExtendedTransmon.close_all()  # ensure no other transmon objects are instantiated
    node_1 = ResonatorSpectroscopy1Node(CONFIG.run.qubits, CONFIG.run.couplers)
    assert issubclass(node_1.measurement_type, ScheduleNode)


def test_measurement_2_type():
    ExtendedTransmon.close_all()  # ensure no other transmon objects are instantiated
    node_2 = ResonatorSpectroscopy2Node(CONFIG.run.qubits, CONFIG.run.couplers)
...
def test_dummy_1_generation():
    ExtendedTransmon.close_all()  # ensure no other transmon objects are instantiated
    node = ResonatorSpectroscopy1Node(CONFIG.run.qubits, CONFIG.run.couplers)
...
def test_dummy_2_generation():
    ExtendedTransmon.close_all()  # ensure no other transmon objects are instantiated
    node = ResonatorSpectroscopy2Node(CONFIG.run.qubits, CONFIG.run.couplers)
