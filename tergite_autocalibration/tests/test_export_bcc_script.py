...
def test_export_bcc_script(tmp_path, redis_connection):
    """
    ...
    with loaded_redis(redis_connection, _redis_backup_path):
        export(["q11", "q12"], ["q11_q12"], redis_connection, output_path=output_path_)
...
def test_export_bcc_script_partial(tmp_path, redis_connection):
    """
    Check whether results are equal to mock values
    """
    output_path_ = os.path.join(tmp_path, "calibration_seed.toml")
    with loaded_redis(redis_connection, _redis_backup_path):
        export(["q11"], [], redis_connection, output_path=output_path_)
